...
struct persistent_seg_tree {
 private:
  int N;
  vertex *buildUtils(vector<int> &vals, int l, int r) {
    if (l == r) {
      return new vertex(vals[l]);
    }
    int mid = (l + r) >> 1;
    return new vertex(buildUtils(vals, l, mid), buildUtils(vals, mid + 1, r));
  }

  vertex *pointUpdateUtils(vertex *node, int l, int r, int pos, int newVal) {
    if (l == r) return new vertex(newVal);
    int mid = (l + r) >> 1;
    if (pos <= mid) {
      return new vertex(pointUpdateUtils(node->leftChild, l, mid, pos, newVal),
                        node->rightChild);
    } else {
      return new vertex(
          node->leftChild,
  ...
  ll rangeSumQueryUtils(vertex *node, int l, int r, int ql, int qr) {
 ...
 public:
  persistent_seg_tree() {}
  vertex *build(vector<int> &vals) {
    N = vals.size();
    return buildUtils(vals, 0, N - 1);
  }

  vertex *pointUpdate(vertex *node, int pos, int newVal) {
    return pointUpdateUtils(node, 0, N - 1, pos, newVal);
  }

  ll rangeSumQuery(vertex *node, int ql, int qr) {
...
void work() {
  ...
  deb(pst.rangeSumQuery(roots[0], 0, 3));
  deb(pst.rangeSumQuery(roots[0], 1, 2));
  roots.push_back(pst.pointUpdate(roots[0], 2, 5));
  deb(pst.rangeSumQuery(roots[1], 0, 3));
  deb(pst.rangeSumQuery(roots[1], 1, 2));
  roots.push_back(pst.pointUpdate(roots[0], 1, 5));
  deb(pst.rangeSumQuery(roots[2], 0, 3));
  deb(pst.rangeSumQuery(roots[2], 1, 2));
  return;
}
